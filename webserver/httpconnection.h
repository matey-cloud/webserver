...
{
public:
    static       int sm_epollfd;                       //所有socket上的事件都被注册到同一个epoll内核事件中，所以设置成静态的
    static       int sm_user_count;                    //统计用户的数量
...
private:
    ...
    HTTP_CODE doRequest();
    inline char *getLine(){ return m_read_buf+m_start_line; }
    ...
    struct sockaddr_in  m_addr;                        //该HTTP连接对方的socket地址
    ...
    char                m_reafile[FILENAME_LEN];       //客户请求的目标文件的完整路径，其内容等于 doc_root + m_url, doc_root是网站根目录
    char               *m_url;                         //客户请求的目标文件的文件名
    char               *m_version;                     //HTTP协议版本号，我们仅支持HTTP1.1
    char               *m_host;                        //主机名
    int                 m_content_len;                 //HTTP请求的消息总长度
    bool                m_is_keep_link;                //HTTP请求是否要求保持连接
